...
(** The player type represents the record fields of the main player, which
    include location, speed, frame, steps, width, and height. *)
...
(** The obstacle type represents the non-moving obstacles of the game and it
    also includes a record field defining their location. *)

type moving =
  | Log
  | Car

(** The moving type represents the moving objects/obstacles. In this game, it is
    a car. *)
...
(** The direction type represents the direction the cars move in. Right is
    moving in the positive x direction and Left is moving in the negative x
    direction. *)
...
(** [check_coll l cs] checks if the main player collides with a moving object.
    If it does, the object is returned. If not, None is returned. *)
...
(** [car type x y t s f d] initializes a car object with fields ob_type,
    location, time, speed, frame, and direction. *)

val draw_car : moving_ob -> unit -> unit
(** [draw_car c] draws a car at the x and y coordinates specified by its
    location field. *)
