...
(*[draw_start_screen] draws the start_screen when the record field game_state is
  set to Start. This displays text asking for an input to start the game*)
...
(*[update_screen] checks the game state and executes the screen accordingly. For
  example, if game_state is equal to fail, it will draw the fail screen *)
